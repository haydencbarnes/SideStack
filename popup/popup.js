...
async function loadTabsFromCache() {
  try {
    const response = await chrome.runtime.sendMessage({ type: 'GET_CACHED_TABS' });
    if (response.ok && response.cache) {
      ...
      // Keep expanded group state local (not persisted in spaces)
      renderTabs();
...
function renderTabs() {
  ...
  // Simplified: no space-based pinned or suspended tabs
...
async function groupTabsByDomain() {
  // Simplified domain grouping without space context
...
function closeContextMenu() {
  ...
  state.context = { target: null, data: null, type: null };
}
...
function buildTabOptions({ tab, isSuspended }) {
  const isPinned = tab.pinned; // Simplified since spaces are removed
  const options = [
    {
      label: isPinned ? 'Unpin' : 'Pin',
      onSelect: () => (isPinned ? unpinTab(tab.id) : pinTab(tab.id)),
    },
    {
  ...
  // Only show "Activate" option for suspended tabs
  if (isSuspended) {
    options.push({
      label: 'Activate',
      onSelect: () => chrome.tabs.update(tab.id, { active: true }),
      disabled: !!tab.active,
    });
  }
  ...
  options.push(
    ...
    {
      label: 'Copy URL',
      onSelect: () => navigator.clipboard.writeText(tab.url),
    },
    {
      label: 'Duplicate',
      onSelect: async () => {
    ...
    {
      label: 'Reload',
      onSelect: () => chrome.tabs.reload(tab.id),
    },
    {
      label: 'Group Tabs by Domain',
      onSelect: groupTabsByDomain,
    },
  );
